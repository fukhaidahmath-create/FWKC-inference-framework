###############################################################################
# FWKC V27 REVIEWER-PRIORITY 2-DAY STRONG CONTINUATION
#
# PURPOSE
# -------
...
# ---------------------------
# - This is NOT the maximum-precision publication preset.
# - The full 243-scenario n x p x c primary grid is preserved.
# - Coverage is evaluated on 120 INDEPENDENT DGM datasets per scenario.
# - The primary inner bootstrap is B=29 for deadline feasibility, while explicit B=100/200/500
#   sensitivity variants remain in the sensitivity tier.
# - Four Bayesian chains, Jeffreys Beta posteriors, posterior-predictive
#   pseudo-raters, RF=300, inverse-SE fusion, and all estimand equations remain.
# - Monte-Carlo precision is reported transparently; no publication QC threshold
...
config <- list(
  ...
  script_version = "v27_reviewer_2day_strong_fixed",
  seed = 20260912,

  # Execution
  run_analysis = TRUE,
  run_mode = "reviewer_priority",  # dedicated deadline reviewer-response run
  # Publication profile:
...
if (config$run_mode == "test") {
...
} else if (config$run_mode == "reviewer_priority") {
  # -------------------------------------------------------------------------
  # REVIEWER-PRIORITY 2-DAY STRONG PRESET
  #
  # SCIENTIFIC ALLOCATION:
  #   1) KEEP the complete 243-scenario n x p x c grid.
  #   2) KEEP M=120 independent DGM datasets/scenario so nominal 95% coverage
  #      MCSE remains ~0.0199 (<=0.020).
  #   3) KEEP 4 Bayesian chains and RF=300.
  #   4) KEEP B=29 as the full-grid baseline and retain explicit B=100/200/500
  #      stress sensitivity so low-B conclusions are directly audited.
  #   5) REUSE the completed V25 high-precision HMC cross-check and B=499
  #      real-data c-grid instead of recomputing them.
  #   6) Reduce only numerical integration and secondary-replication burden.
  #
  # IMPORTANT:
  #   This is a two-day TARGET on the observed workstation, not a wall-clock
  #   guarantee. The code reports exact elapsed times and checkpoints all stages.
  # -------------------------------------------------------------------------

  config$bayes_engine <- "gibbs"
  config$bayes_chains <- 4L

  # Baseline MC integration is deliberately lighter, but higher n_mc values
  # (250/500/1000/2000) remain in sensitivity analyses.
  config$n_mc <- 150L
  ...
  config$bayes_thin <- 1L

  # Reviewer-facing ML specification remains unchanged.
  config$rf_ntree <- 300L

  # PRIMARY reviewer-facing frequentist coverage experiment.
  config$B_boot <- 29L
  config$B_boot_real <- 499L       # imported from completed V25, not recomputed
  config$B_boot_dgm <- 29L
  config$B_boot_sim <- config$B_boot_dgm
  config$M_dgm <- 120L
  config$scenario_design <- "full_factorial"
  config$scenario_cap <- NA_integer_

  # Expensive BCa is not the primary FWKC interval. Higher-B stress checks remain.
  config$compute_bca <- FALSE
  config$covariance_boot_B <- 39L

  # Large-sample n=620 bridge: full c-grid retained, lighter secondary replication.
  config$anchor_bridge_M <- 15L
  config$anchor_bridge_B <- 29L
  ...
  # Higher n_mc and B variants directly audit the reduced deadline baseline.
  config$run_sensitivity <- TRUE
  config$sensitivity_M <- 10L
  config$sensitivity_B <- 29L
  config$sensitivity_n_mc <- 150L
  config$sensitivity_scenarios <- 3L

  # Keep all 14 verified AuditC studies and all nine c values.
  config$B_boot_external <- 49L
  config$external_validation_source <- "AuditC_builtin_verified"
  config$external_max_studies <- NA_integer_
  config$external_validation_required_in_publication <- FALSE

  # Computational benchmark only.
  config$run_scalability <- TRUE
  config$scalability_reps <- 1L
  config$scalability_B <- 15L
  config$scalability_n_mc <- 80L

  # DO NOT rerun HMC. V27 imports the completed V25 same-prior PASS object.
  config$run_bayes_engine_crosscheck <- FALSE
  config$require_bayes_crosscheck_in_publication <- FALSE
  ...
  # Resume/durability.
  config$checkpoint_every_dgm <- 20L
...
build_reviewer_priority_qc <- function(
  real_data, main_results, anchor_bridge, anchor_sample_check, robustness,
  sensitivity, external, bayes_crosscheck, cfg
) {
  checks <- list(); k <- 0L
  add <- function(name, value, threshold, pass, note) {
    k <<- k + 1L
    checks[[k]] <<- data.frame(
      check=name, value=as.character(value), threshold=as.character(threshold),
      pass=isTRUE(pass), note=note, stringsAsFactors=FALSE
  ...
  fmin <- function(x) { x<-x[is.finite(x)]; if(!length(x)) NA_real_ else min(x) }
  fmax <- function(x) { x<-x[is.finite(x)]; if(!length(x)) NA_real_ else max(x) }

  add("reviewer_priority_mode", cfg$run_mode, "reviewer_priority",
      identical(cfg$run_mode,"reviewer_priority"),
      "This gate is for the deadline reviewer-response run; it is not the V25 maximum-precision publication gate.")
  ...
  real_grid_ok <- !is.null(rr) && nrow(rr)>=9 &&
    setequal(round(unique(rr$c_val),10), round(seq(.1,.9,.1),10))
  ...
  full_grid <- !is.null(main_results) && nrow(main_results)>=243 &&
    ...
    setequal(round(unique(main_results$c_val),10),round(seq(.1,.9,.1),10)) &&
    setequal(unique(main_results$n),c(30,50,100))
  add("complete_primary_9x9x3_grid", if(is.null(main_results)) 0 else nrow(main_results),
      ">=243 complete anchor scenarios", full_grid,
      "Continuity with the submitted manuscript is preserved across n=30,50,100 and the full p-by-c grid.")

  mcse_nom <- sqrt(cfg$nominal_coverage*(1-cfg$nominal_coverage)/cfg$M_dgm)
  add("independent_dgm_design", paste0("M=",cfg$M_dgm,"; nominal_MCSE=",signif(mcse_nom,4)),
      "M>=120 independent datasets/scenario and nominal MCSE<=0.020; report MCSE/Wilson intervals",
      cfg$M_dgm>=120 && is.finite(mcse_nom) && mcse_nom<=0.020,
      "This directly corrects the submitted n_DGM=1 coverage flaw. Precision is lower than V25 M=500 and must be reported transparently.")

  add("inner_bootstrap_design", paste0("B=",cfg$B_boot_dgm),
      "B>=29 plus B=100/200/500 stress sensitivity", cfg$B_boot_dgm>=29,
      "The reviewer-priority baseline uses a smaller inner bootstrap; higher-B sensitivity variants audit dependence of conclusions on B.")

  retained_draws_per_chain <- floor((cfg$bayes_iter - cfg$bayes_burn) / max(1L,cfg$bayes_thin))
  add("core_numerical_design",
      paste0("n_mc=",cfg$n_mc,
             "; chains=",cfg$bayes_chains,
             "; iter/burn/thin=",cfg$bayes_iter,"/",cfg$bayes_burn,"/",cfg$bayes_thin,
             "; retained/chain=",retained_draws_per_chain,
             "; RF=",cfg$rf_ntree),
      "n_mc>=150 + higher-n_mc sensitivity; 4 chains; iter>=400; retained>=300/chain; RF>=300",
      cfg$n_mc>=150 && cfg$bayes_chains>=4 && cfg$bayes_iter>=400 &&
        retained_draws_per_chain>=300 && cfg$rf_ntree>=300,
      "Deadline baseline reduces MC integration but uses no thinning and retains 300 Gibbs draws/chain; n_mc=250/500/1000/2000 sensitivity audits numerical stability.")

  if(!is.null(main_results) && nrow(main_results)>0) {
    md <- fmin(main_results$direct_dataset_success_rate)
    mf <- fmin(main_results$fwkc_dataset_success_rate)
    bc <- fmin(main_results$bayes_convergence_rate)
    add("primary_fit_success", paste0("Direct=",signif(md,4),"; FWKC=",signif(mf,4)),
        ">=0.90 both", is.finite(md)&&md>=.90&&is.finite(mf)&&mf>=.90,
        "Failures remain visible and are not conditioned away.")
    add("primary_bayesian_convergence", bc, ">=0.95", is.finite(bc)&&bc>=.95,
        "The accelerated blocked/collapsed Gibbs run must still demonstrate stable convergence across the primary grid.")
    if("mcse_fwkc"%in%names(main_results)) {
      add("observed_max_fwkc_coverage_mcse", fmax(main_results$mcse_fwkc),
          "reported, not hidden", TRUE,
          "This reviewer-priority run reports actual scenario-level Monte Carlo error instead of pretending to meet the V25 M=500 precision gate.")
    }
  }

  an_ok <- !is.null(anchor_sample_check) && nrow(anchor_sample_check)>0 && isTRUE(anchor_sample_check$exact_match[1])
  abr <- if(!is.null(anchor_bridge) && is.list(anchor_bridge)) anchor_bridge$results else NULL
  abr_ok <- !is.null(abr) && nrow(abr)>0 && all(abr$n==cfg$manuscript_anchor_n) &&
    setequal(round(unique(abr$c_val),10),round(cfg$anchor_bridge_c_values,10))
  add("anchor_n620_bridge", paste0("sample_check=",an_ok,"; bridge=",abr_ok),
      "TRUE/TRUE", an_ok&&abr_ok,
      "The real-data sample size remains n=620 and a separate large-sample bridge is retained.")

  robust_types <- if(!is.null(robustness)&&nrow(robustness)>0) unique(robustness$dgm_type) else character(0)
  add("noncircular_multiple_dgms", length(robust_types), ">=3 structural DGM types",
      length(robust_types)>=3,
      "This addresses the reviewer concern that validation should not be circularly anchored to one DGM.")
  ...
  labs <- if(!is.null(sensitivity)&&"sensitivity_label"%in%names(sensitivity)) unique(as.character(sensitivity$sensitivity_label)) else character(0)
  sens_complete <- all(required_labels %in% labs)
  mvalid <- if(!is.null(sensitivity)&&"M_valid"%in%names(sensitivity)) sensitivity$M_valid else numeric(0)
  sens_valid <- sens_complete && length(mvalid)==nrow(sensitivity) && all(is.finite(mvalid)) && all(mvalid>0)
  add("full_sensitivity_family", paste0(length(labs)," labels"), paste0(length(required_labels)," required + all M_valid>0"),
      sens_valid,
      "Breadth of sensitivity is preserved even though each stress cell uses fewer DGM repetitions.")

  ext_ok <- !is.null(external) && nrow(external)>0 && all(c("dataset","c_val","fit_success")%in%names(external))
  ds <- if(ext_ok) unique(as.character(external$dataset)) else character(0)
  ext_grid <- ext_ok && length(ds)>=14 && all(vapply(ds,function(d)
    setequal(round(unique(external$c_val[external$dataset==d]),10),round(cfg$external_c_values,10)),logical(1)))
  ext_success <- if(ext_ok) min(aggregate(fit_success~dataset,external,function(z) mean(as.numeric(z),na.rm=TRUE))$fit_success,na.rm=TRUE) else NA_real_
  add("external_cross_domain_real_data", paste0("datasets=",length(ds),"; min_success=",signif(ext_success,4)),
      "14 studies; full c grid; min success>=0.90",
      ext_grid && is.finite(ext_success) && ext_success>=.90,
      "External AuditC evidence is cross-domain generalizability, not same-disease coronary transport validation.")

  out <- rbind_fill(checks)
  save_csv_safe(out,file.path(cfg$output_dir,"REVIEWER_PRIORITY_QC.csv"))
  out
}

write_reviewer_priority_gate <- function(qc, cfg) {
  ok <- !is.null(qc) && nrow(qc)>0 && all(qc$pass %in% TRUE)
  ready <- file.path(cfg$output_dir,"REVIEWER_PRIORITY_READY.flag")
  fail <- file.path(cfg$output_dir,"REVIEWER_PRIORITY_REVIEW_NEEDED.flag")
  if(file.exists(ready)) file.remove(ready)
  if(file.exists(fail)) file.remove(fail)
  stamp <- paste0("run_mode=",cfg$run_mode,"\n",
...
write_summary_workbook <- function(
  ...
  saveWorkbook(wb,file.path(cfg$output_dir,"FWKC_FINAL_V27_REVIEWER_2DAY_SUMMARY.xlsx"),overwrite=TRUE)
}
...
main <- function(cfg) {
  ...
  safe_write_lines(
    c(
      ...
      paste0("reviewer_priority_source_dir: ", if(!is.null(cfg$reviewer_priority_source_dir)) cfg$reviewer_priority_source_dir else NA_character_),
      paste0("reviewer_priority_target_days: ", if(!is.null(cfg$reviewer_priority_target_days)) cfg$reviewer_priority_target_days else NA_integer_)
    ),
  ...
  if (identical(cfg$run_mode,"reviewer_priority")) {
    ...
    log_message(
      ...
      " | target_days=",cfg$reviewer_priority_target_days
    )
  }
  ...
  if (identical(cfg$run_mode,"reviewer_priority") &&
    ...
    reuse_real_ok <- !is.null(rr_reuse) && nrow(rr_reuse)>=9 &&
      setequal(round(unique(rr_reuse$c_val),10),round(seq(.1,.9,.1),10))
    if (!reuse_real_ok) stop("Saved V25 real-data stage is incomplete; expected the full c=0.1,...,0.9 grid.")
    dir.create(file.path(cfg$output_dir,"real_data"),recursive=TRUE,showWarnings=FALSE)
    save_csv_safe(rr_reuse,file.path(cfg$output_dir,"real_data","real_data_results_REUSED_V25.csv"))
    if(!is.null(real_data$paired_tests) && nrow(real_data$paired_tests)>0)
  ...
  publication_qc <- if (identical(cfg$run_mode,"reviewer_priority")) {
    build_reviewer_priority_qc(
  ...
  reportable <- if (identical(cfg$run_mode,"reviewer_priority")) {
    write_reviewer_priority_gate(publication_qc,cfg)
  } else {
    write_reportability_gate(publication_qc,cfg)
  }
  mark_main_stage("plots + workbook + reviewer-priority/reportability gate")
  ...
  safe_save_rds(all_results,file.path(cfg$output_dir,"FWKC_FINAL_V27_REVIEWER_2DAY_STRONG_ALL_RESULTS.rds"))
...
if (isTRUE(config$run_analysis)) {
  message(
    "FWKC V27 reviewer-priority continuation is starting in run_mode = ",
    config$run_mode,
